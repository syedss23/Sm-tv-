// episode.js (updated - opens custom player page for watch3)
...
Promise.all([
  ...
  function renderEpisode() {
    ...
    // Lazy loading iframe optimization
    ...
    if (download2Btn && ep.download2) {
      download2Btn.addEventListener("click", (e) => {
        e.preventDefault();
        window.location.href = ep.download2;
      });
    }

    // ✅ Updated Watch Server 3 - open new player page
    const watch3Btn = document.getElementById("watch3Btn");
    if (watch3Btn && (ep.watch3 || ep.embed3)) {
      watch3Btn.addEventListener("click", (e) => {
        e.preventDefault();
        const playerUrl = `player.html?series=${slug}&season=${season || ''}&ep=${ep.ep}`;
