...
// Get series, but fall back to 'salahuddin-ayyubi-s2' if not present (adjust if needed)
let slug = params.get('series') || 'salahuddin-ayyubi-s2';   // <-- Edit this default if needed!
const epSlug = params.get('ep'); // This should be the actual episode slug, e.g. 'salahuddin-ayyubi-s2e1'

const HOW_TO_DOWNLOAD_URL = "https://t.me/howtodownloadd1/10";
const PREMIUM_CHANNEL_URL = "https://t.me/itzmezain1/2905";

// Always look for 'episode-data/slug.json'
function getEpisodeFile(slug) {
  return `episode-data/${slug}.json`;
}

fetch('series.json').then(r => r.ok ? r.json() : []).then(seriesList => {
  // fallback if missing series.json/metadata
  const meta = Array.isArray(seriesList) ? seriesList.find(s => s.slug === slug) : null;
  fetch(getEpisodeFile(slug))
    .then(r => r.ok ? r.json() : [])
    .then(episodesArray => {
