document.addEventListener('DOMContentLoaded', () => {
  ...
  if (grid) {
    ...
    function hydrateUI() {
      ...
      // ensure active lang styling (only matters after Subtitles is clicked)
    ...
    function toggleSubLangs() {
      if (!subLangs) return;
      // Hide language pills on Home by default; only show when Subtitles is chosen
    ...
    // Mobile-friendly instant search (filters current view only)
    ...
    // Resize-friendly: re-run counts if needed (optional, cheap)
    window.addEventListener('orientationchange', () => setTimeout(setCounts, 300));
  }

  // ---------------- Legacy SPA list (kept for compatibility with older markup) ----------------
